...
def test_compute_response_returns_expected_shapes() -> None:
    ...
    assert response_result.green_operator.dimension == 3
...
def test_green_operator_inverts_coupling_matrix() -> None:
    w, theta = make_symbols(3, 3)
    phi = bootstrap_potential(w, theta)
    grad = gradient_of_potential(phi, w)
    hess = hessian_of_potential(phi, w)
    ...
    C = response_result.coupling_matrix
    G = response_result.green_operator
    I = np.eye(C.shape[0], dtype=float)
    assert np.allclose(C @ G.apply(I), I)

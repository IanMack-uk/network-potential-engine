...
def test_check_pointwise_conditions_returns_expected_flags_for_bootstrap_model() -> None:
    ...
    assert result.is_c_strictly_diagonally_dominant is True
    assert result.has_c_nonpositive_off_diagonals is True
    assert result.response_is_entrywise_nonnegative is True

    assert result.c_diagonal_dominance_margins.shape == (3,)
    assert np.allclose(result.c_diagonal_dominance_margins, np.array([1.0, 1.0, 1.0]))
    assert np.isclose(result.c_min_diagonal_dominance_margin, 1.0)

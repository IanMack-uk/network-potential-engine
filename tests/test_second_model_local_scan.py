...
def test_second_model_local_scan_returns_records() -> None:
    ...
    assert np.allclose(records[0].theta, np.array([0.1, 0.0, -0.1]))
    assert np.allclose(records[1].theta, np.array([0.2, 0.1, 0.0]))
    assert np.allclose(records[2].theta, np.array([0.3, 0.2, 0.1]))


def test_second_model_local_scan_pointwise_checks_pass() -> None:
    w, theta = make_symbols(3, 3)
    ...
    assert minimum_scan_margin(records) > 0.0


def test_second_model_local_scan_equilibrium_differences_nonnegative() -> None:
    w, theta = make_symbols(3, 3)

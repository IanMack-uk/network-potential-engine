...
def check_tdc_inverse_positivity_example() -> None:
    ...
    tol = 1e-12
    min_entry = float(np.min(inv_coupling))
    assert min_entry >= -tol, f"Inverse positivity failed: min entry = {min_entry}"


def run_all_checks() -> None:
    print("Running D1 inverse-positivity checks (TDC model)")

    check_tdc_inverse_positivity_example()
    print("✓ C(θ)^(-1) is entrywise nonnegative for a representative θ")

    print("D1 checks completed successfully")

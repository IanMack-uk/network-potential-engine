...
def check_response_identity_at_equilibrium() -> None:
    ...
    assert residual == sp.zeros(*jac.shape), (
        "C1 response identity failed: D_theta w*(theta) != C(theta)^(-1) H_wtheta(w*(theta), theta)."
    )


def run_all_checks() -> None:
    print("Running C1 equilibrium response checks (TDC model)")

    check_response_identity_at_equilibrium()
    print("✓ Dθw*(θ) = C(θ)^(-1) H_wθ(w*(θ), θ) verified")

    print("C1 checks completed successfully")

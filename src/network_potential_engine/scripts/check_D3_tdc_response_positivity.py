...
def check_tdc_response_positivity_example() -> None:
    ...
    tol = 1e-12
    min_entry = float(np.min(response))
    assert min_entry >= -tol, f"Response positivity failed: min entry = {min_entry}"


def run_all_checks() -> None:
    print("Running D3 response positivity checks (TDC model)")

    check_tdc_response_positivity_example()
    print("✓ Dθw*(θ) is entrywise nonnegative for a representative θ")

    print("D3 checks completed successfully")
